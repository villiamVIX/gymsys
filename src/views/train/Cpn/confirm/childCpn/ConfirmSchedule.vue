	...
	<div id="ConfirmSchedule">
		<div class="Appointment">
			<div v-for="(i,indexW) in dateList" :key="indexW" class="week-box" :class="{'passday':i.limit}" @click="!i.limit?showSegment(i,indexW):''">
				...
				<div v-for="(s,indexS) in Segment" :key='indexS' @click="clickDay(s,indexW,indexS,i,$event)" class="noDisplay"
				 :class="indexW== currentShow? 'SegmentShow':''">
					<div class="Segment-item" :id="i.configDate+'-'+s" :class="indexS==currentIndexS?'Segment-hover':''">
						{{s}}:00
						<br />
						<span :class="indexS==currentIndexS?'book-hover':''" class="book-status">可约</span> </div>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			filterBooked() { //改为接手唯一日期 而不是Sid
				let k = this.booked
				let kL = k.length
				for (let i = 0; i < kL; i++) {
					let bookedDom = document.getElementById(k[i])
					bookedDom.innerText = '已预约'
					bookedDom.classList.add('booked')
				}
			},
